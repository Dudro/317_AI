...
def is_goal(state):
    for car in range(state.get_number_of_cars()):
        if state.get_car_loc(car) != state.get_world().get_garage():
            return False
    return all_packages_delivered(state)


def all_packages_delivered(state):
    """
    Determines whether the given state has all packages delivered.

    :param state: a particular state
    :type state: State
    :rtype: bool
    """
    return state.get_num_undelivered() == 0
